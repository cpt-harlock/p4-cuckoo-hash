...
control MyIngress(inout headers hdr,
	...
	apply {
		bit<TABLES_KEY_INPUT_WIDTH> packet_key;
		bit<TABLES_VALUE_INPUT_WIDTH> packet_value;
		...
		// key always found in the AXI Stream tuser field
		packet_key = meta.axis_tuser[TABLES_KEY_INPUT_WIDTH-1:0];
		packet_value = meta.axis_tuser[(TABLES_KEY_INPUT_WIDTH + TABLES_VALUE_INPUT_WIDTH)-1:TABLES_KEY_INPUT_WIDTH];


		// compute index for first table
		hash_first_level_first_datapath_input_key = packet_key;
		...
		// populate first ch input
		ch_first_level_first_datapath_input_index = hash_first_level_first_datapath_output;
		ch_first_level_first_datapath_input_key = packet_key;
		ch_first_level_first_datapath_input_value = packet_value;
		// evict if recirculating
		ch_first_level_first_datapath_input_evict = meta.axis_tdest;
		// never ignore on first datapath
		ch_first_level_first_datapath_input_ignore_input = 0;
		...
		stash_threshold = stash_first_datapath_output_counter >= 1;
		// a try
		// nested if should work for variables
		bit<2> recirc_input = FLAG_INPUT_READ;
		if (meta.axis_tdest == 0) {
			if (stash_threshold && stash_first_datapath_output_discarded == 0)
				recirc_input = FLAG_INPUT_SET;
		}
		else {
			if (meta.axis_tid >= LOOP_LIMIT || stash_first_datapath_output_discarded == 1 ) {
		...
		if (meta.axis_tdest == 0) {
			if (recirculating_flag_read == 0 && stash_threshold && stash_first_datapath_output_discarded == 0) {
				meta.axis_tdest = 1;
				meta.axis_tid = 0;
			}	
		}
		else {
			if (stash_first_datapath_output_discarded == 1) {
